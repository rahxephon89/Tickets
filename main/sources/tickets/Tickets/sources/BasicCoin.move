module Sender::BasicCoin {
    ...
    public(script) fun publish_balance_script(account: signer) {
    ...
    /// Mint `amount` tokens to `mint_addr`. Mint must be approved by the module owner.
    public(script) fun mint_script(module_owner: signer, mint_addr: address, amount: u64) acquires Balance {
        // Only the owner of the module can initialize this module
        assert!(Signer::address_of(&module_owner) == MODULE_OWNER, Errors::requires_address(ENOT_MODULE_OWNER));
    ...
    public(script) fun transfer_script(from: signer, to: address, amount: u64) acquires Balance {
